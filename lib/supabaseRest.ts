...
function normalizeCert(value: string) {
  return decodeURIComponent(value).trim().replace(/^#/, "").replace(/\s+/g, "").toUpperCase();
}
...
function certCandidates(value: string) {
  ...
  return Array.from(new Set([normalized, numeric, `ASG${numeric}`, `ASG-${numeric}`, padded, `ASG${padded}`, `ASG-${padded}`].filter(Boolean)));
}
...
export async function fetchCertificateByNumber(value: string) {
  ...
  for (const candidate of candidates) {
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/certificates?select=*&cert_number=eq.${encodeURIComponent(candidate)}&status=eq.Verified&limit=1`,
      { headers: { apikey: SUPABASE_PUBLISHABLE_KEY }, cache: "no-store" }
    );
    if (!response.ok) throw new Error(`Certificate lookup failed (${response.status}): ${await response.text()}`);
...
export async function verifyAsgAdmin(accessToken: string) {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/asg_admins?select=user_id,role&limit=1`, {
...
export async function uploadCertImage(accessToken: string, certNumber: string, side: "front" | "back", file: File) {
